...
Most toolkit steps are identical after you have `sudo`, but a **fresh Debian
install often does not ship `sudo`**, and the first user is usually **not** in
the `sudo` group (unlike typical Ubuntu cloud images). Do this once as **root**
before the normal install.

**1) Bootstrap `sudo` (as root)** — log in as `root`, or run `su -` from your
user account, then:

```bash
# As root on the Debian VM:
apt-get update
...
Log out and back in as `YOUR_USER` so the new group membership applies
(`groups` should list `sudo`). After that, the standard toolkit commands that
use `sudo …` work the same as on Ubuntu.

**2) Fresh local install (as YOUR_USER, with sudo)** — same as the Start here
block, once `sudo` works:
...
Other Debian friction points:

- **Local HTTPS / mkcert (on the HOST browser machine):** `apt install -y mkcert libnss3-tools` before `mkcert -install`, so NSS-based browsers can trust the certificate. Firefox may still need the steps in `browser-trust-guide`.
