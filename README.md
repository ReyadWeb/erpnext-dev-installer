...
v0.5.4 adds an optional App Library for installing common Frappe ecosystem apps into the local ERPNext developer VM.
...
### v0.5.4 readiness polish

v0.5.4 makes service start and restart clearer. After `start`, `restart`, `service-start`, or `service-restart`, the script shows visible waiting output while checking required development ports. This prevents confusion when systemd reports the service as running but Bench is still starting internally.
...
v0.5.4 keeps the main menu simple:
...
v0.5.4 can create a local development systemd service:
